...
const SectionEditor = ({ section, onChange, onRemove, onMoveUp, onMoveDown, index }) => {
  ...
  return (
    <div className="border border-gray-200 rounded-md mb-4 overflow-hidden shadow-sm hover:shadow-md transition-shadow duration-200">
      <div 
        className={`flex justify-between items-center p-4 ${expanded ? 'bg-blue-50 border-b border-blue-200' : 'bg-gray-50'} cursor-pointer`}
        onClick={() => setExpanded(!expanded)}
      >
        <div className="flex items-center flex-1 min-w-0">
          <div className="flex items-center justify-center w-7 h-7 bg-blue-100 text-blue-800 rounded-full font-bold text-sm mr-3 flex-shrink-0">
            {index + 1}
          </div>
          <div className="flex flex-col min-w-0">
            <div className="flex items-center">
              {(() => {
                switch(section.type) {
                  ...
                  case 'exercise':
                    return (
                      <span className="flex items-center text-pink-700 font-medium">
                        <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            ...
            {(section.title || section.caption || section.content) && (
              <p className="text-sm text-gray-500 truncate mt-1">
                {section.title || section.caption || (section.content && section.content.substring(0, 40) + (section.content.length > 40 ? '...' : ''))}
              </p>
            )}
          </div>
        </div>
        <div className="flex items-center ml-4">
          <div className="flex bg-white rounded-md shadow-sm border border-gray-200 mr-2">
            <button
              type="button"
              onClick={(e) => {
                e.stopPropagation();
              ...
              className="p-1.5 text-gray-500 hover:bg-gray-50 hover:text-blue-500 border-r border-gray-200"
              title="Move up"
            >
            ...
            <button
              ...
              className="p-1.5 text-gray-500 hover:bg-gray-50 hover:text-blue-500"
              title="Move down"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
          ...
          <button
            ...
            className={`p-1.5 rounded-md ml-1 ${expanded ? 'bg-blue-100 text-blue-500' : 'text-gray-400 hover:bg-gray-100'}`}
