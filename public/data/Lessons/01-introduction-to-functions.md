...
print(len(long_gazes)) # Output: 14
```
...
def analyze_gaze_data(filename):
    ...
    long_gazes = [d for d in durations if d > 600]
    percentage = (len(long_gazes) / len(durations)) * 100
    return percentage

result = analyze_gaze_data('eye_tracking_data.txt')
print(result) # Output: Approximately 35.0
```
...
def gaze_analysis_stats(filename):
    ...
    std_dev = 0
    if long_gazes:
        mean = sum(long_gazes) / len(long_gazes)
        variance = sum([(x - mean)**2 for x in long_gazes]) / len(long_gazes)
        std_dev = math.sqrt(variance)

    return {'valid_count': len(durations), 'average': avg, 'std_dev': std_dev}
